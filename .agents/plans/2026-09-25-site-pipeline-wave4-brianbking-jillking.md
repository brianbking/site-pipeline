...
- [ ] **Step 6: Preview URLs toggle** on the production Worker. Verify in Brian's terminal: `npx wrangler versions upload --env=""` prints a `Version Preview URL:` line. Then check that URL **by hand**: `<preview>/` returns 200, and `<preview>/resume/` returns **404 `Not found`**.
